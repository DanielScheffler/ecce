:- module(msg, [msg/3, is_variant_of/2]).
/********************************************************************************/
/* 
MODULE MSG
...
msg(X,Y,MSG) :- construct_msg(X,Y,MSG).
...
is_variant_of( _term1, _term2 ):-
    instance_of( _term1, _term2, [], _, _flag ),
    _flag \== less_general.
...
combi_instance_subterms( [ _term1| _terms1 ], [ _term2| _terms2], _inrec, _outrec, _flag, [ _mask1| _masks] ):-
    ...
    instance_of( _term1, _term2, _tmprec, _outrec, _flag ).
...
is_more_general( _term1, _term2 ):-
    instance_of( _term2, _term1, [], _, _flag ).

  
instance_of( _term1, _term2, _record, _record, _ ):-
    subst_recorded( _term2, _record, _term3 ), !, _term1 == _term3.
instance_of( _term1, _term2, _record, _record, _ ):-
    atomic( _term2 ), !, _term1 == _term2.
instance_of( _term1, _term2, _inrec, _outrec, _flag ):-
    nonvar( _term2 ), functor( _term2, _functor, _arity ), !,
    nonvar( _term1 ), functor( _term1, _functor, _arity ),
    _term1 =.. [_|_subterms1],
    _term2 =.. [_|_subterms2], 
    instance_subterms( _subterms1, _subterms2, _inrec, _outrec, _flag ).
instance_of( _term1, _term2, _record, [_subst|_record], less_general ):-
...
instance_of( _term1, _term2, _record, [_subst|_record], less_general ):-
    ...
    _subst = subst(_term2,_term1).
instance_of( _term1, _term2, _record, [_subst|_record], _ ):-
    var(_term2), var(_term1), 
    _subst = subst(_term2,_term1).
...
construct_subst_list( _atom, _instatom, _varlist, _substlist ) :-
    instance_of( _instatom, _atom, [], _record, _ ),
